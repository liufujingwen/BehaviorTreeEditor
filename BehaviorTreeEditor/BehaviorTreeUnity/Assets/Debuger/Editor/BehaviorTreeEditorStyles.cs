...
namespace BT.Debuger
{
    public static class BehaviorTreeEditorStyles
    {
        ...
        public static GUIStyle GetNodeStyle(NodeDesigner node)
        {
            ...
            if (node.baseNode.Status == BehaviorTree.NodeStatus.READY)
                guiStyle = normalNodeStyle;
            else if (node.baseNode.Status == BehaviorTree.NodeStatus.RUNNING)
                guiStyle = runningNodeStyle;
            else if (node.baseNode.Status == BehaviorTree.NodeStatus.FAILED)
                guiStyle = failedNodeStyle;
            else if (node.baseNode.Status == BehaviorTree.NodeStatus.SUCCESS)
                guiStyle = successedNodeStyle;
            else if (node.baseNode.Status == BehaviorTree.NodeStatus.ERROR)
